...
print("Minion Deployment Handler MMOdev")
print("Initial sample duration: {}".format(INIsamp))
...
if __name__ == '__main__':
    ...
    if samp_count == 0:
# mmo modifed
        print("Entered Extended Sampler")
        os.system('sudo python3 /home/pi/Documents/Minion_scripts/Extended_Sampler_MMOdev.py &')
# mmo needed to add
        secINI = INIsamp*60*60
        time.sleep(secINI)
    elif samp_count >= TotalSamples + 1 or Abort == True:
        GPIO.output(IO328, 0)
        print("Entered Recovery Sampler")
        os.system('sudo python3 /home/pi/Documents/Minion_scripts/Recovery_Sampler_Burn.py &')

    else:
        print("Entered Timelapse Sampler")
        if iniImg == True:
